...
public class PoiExcelParser {
     ...
     */
    public PoiExcelParser parse(InputStream stream) throws InvalidFormatException, IOException, ParseException {
     ...
     * @throws ParseException
     */
    public synchronized PoiExcelParser parse(InputStream stream, int sheetId) throws InvalidFormatException, IOException, ParseException {
        // 每次转换前都清空数据
        datas.clear();
        // 打开表格文件输入流
     ...
     */
    protected XMLReader getSheetParser(StylesTable styles, ReadOnlySharedStringsTable strings) throws ParserConfigurationException, SAXException {
...
//     this.columsLength = columsLength;
    ...
    public static void main(String[] args) throws IOException, ParseException, InvalidFormatException {
        File file = new File("E:\\tmp\\tem.xlsx");
        FileInputStream inputStream = new FileInputStream(file);
        PoiExcelParser excelParser = new PoiExcelParser();
        PoiExcelParser parse = excelParser.parse(inputStream);
        List<String[]> datas = parse.getDatas();
        System.out.println("导入数据条数："+datas.size());
