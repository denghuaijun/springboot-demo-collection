package com.dhj.demo.websocket.handle;
 ...
 */
@Service
public class CustomWebSocketHandler02 extends TextWebSocketHandler implements WebSocketHandler {
    ...
    public void sendMessage(String jsonData) {
        ...
        String user = msgJson.getString("to");
        String msg = msgJson.getString("msg");
        if("all".equals(user.toLowerCase())) {
            sendMessageToAllUsers(new TextMessage( msg));
        }else {
            sendMessageToUser(user, new TextMessage(msg));
        }
    }
